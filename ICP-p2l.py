...
def save_point_cloud(file_path, points, colors):
    data = np.hstack((points, colors * 255))
...
# load data
pcd1 ,colors1= load_point_cloud('./data/16.txt')
pcd2 ,colors2= load_point_cloud('./data/44pitch.txt')

# downsampling
pcd_s,colors_s = voxel_down_sample(pcd1,colors1, voxel_size=0.1)
pcd_t,colors_t = voxel_down_sample(pcd2,colors2, voxel_size=0.1)

# ensure both point cloud share same amount of points after downsampling
def match_point_cloud_size(src, src_colors, dst, dst_colors):
    min_size = min(len(src), len(dst))
    return src[:min_size], src_colors[:min_size], dst[:min_size], dst_colors[:min_size]

pcd_s, colors_s, pcd_t, colors_t = match_point_cloud_size(pcd_s, colors_s, pcd_t, colors_t)  

# show initial status
fig = plt.figure(figsize=(15, 10))
ax = fig.add_subplot(111, projection='3d')
ax.scatter(pcd_s[:, 0], pcd_s[:, 1], pcd_s[:, 2], c=colors_s, marker='o', label='Source (16)')
ax.scatter(pcd_t[:, 0], pcd_t[:, 1], pcd_t[:, 2], c=colors_t, marker='^', label='Target (44)')
...
ax = fig.add_subplot(111, projection='3d')
ax.scatter(pcd_t[:, 0], pcd_t[:, 1], pcd_t[:, 2], c=colors_t, marker='^', label='Target (44)')
ax.scatter(pcd_s_transformed[:, 0], pcd_s_transformed[:, 1], pcd_s_transformed[:, 2], c=colors_s, marker='o', label='Transformed Source')
ax.legend(loc="upper right")
